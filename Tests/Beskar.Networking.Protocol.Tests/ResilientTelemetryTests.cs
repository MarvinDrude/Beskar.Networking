...
public class ResilientTelemetryTests
{
   ...
   [Test]
   public async Task ResilientServer_HandshakeAndDisconnect_TracksServerRoleSessionState()
   {
      long serverRoleSessionDelta = 0;

      using var meterListener = new MeterListener();
      meterListener.InstrumentPublished = (instrument, listener) =>
      {
         if (instrument.Meter.Name == ResilientMetrics.MeterName &&
             instrument.Name == "beskar.resilient.sessions.active")
         {
            listener.EnableMeasurementEvents(instrument);
         }
      };
      ...
      meterListener.SetMeasurementEventCallback<long>((instrument, measurement, tags, state) =>
      {
         ...
         {
            if (tag.Key == "role" && Equals(tag.Value, "server"))
            {
               Interlocked.Add(ref serverRoleSessionDelta, measurement);
            }
         }
      });
      ...
      var endpoint = new MemoryEndPoint($"resilient_server_telemetry_{Guid.NewGuid():N}");
      var listener = new MemoryNetworkListener(endpoint, new MemoryTransportOptions());
      var server = new ResilientServer<CustomMagicPacket>([listener], new ResilientServerOptions());
      await server.StartAsync();
      ...
      await Assert.That(Volatile.Read(ref serverRoleSessionDelta)).IsGreaterThanOrEqualTo(1);
