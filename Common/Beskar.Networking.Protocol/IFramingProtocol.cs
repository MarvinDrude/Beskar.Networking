...
public interface IFramingProtocol<TSelf> where TSelf : struct, IFramingProtocol<TSelf>
{
   ...
   /// </summary>
   public static abstract bool TryRead(ref SequenceReader<byte> reader, out TSelf result);
   ...
   /// </summary>
   public ResilientFrameKind GetFrameKind() => ResilientFrameKind.Message;

   /// <summary>
   /// Optionally creates a protocol frame instance of the specified kind.
   /// </summary>
   public static virtual TSelf CreateFrame(ResilientFrameKind kind) => default;

   /// <summary>
   /// Optionally creates a protocol frame instance of the specified kind with a payload sequence.
   /// </summary>
   public static virtual TSelf CreateFrame(ResilientFrameKind kind, ReadOnlySequence<byte> payload) => TSelf.CreateFrame(kind);

   /// <summary>
   /// Optionally creates a protocol frame instance of the specified kind with a payload memory block.
   /// </summary>
   public static virtual TSelf CreateFrame(ResilientFrameKind kind, ReadOnlyMemory<byte> payload) => TSelf.CreateFrame(kind, new ReadOnlySequence<byte>(payload));

   /// <summary>
   /// Optionally creates a protocol frame instance of the specified kind with a byte array payload.
   /// </summary>
   public static virtual TSelf CreateFrame(ResilientFrameKind kind, byte[] payload) => TSelf.CreateFrame(kind, new ReadOnlySequence<byte>(payload));

   /// <summary>
   /// Optionally creates a message protocol frame with a payload sequence.
   /// </summary>
   public static virtual TSelf CreateMessage(ReadOnlySequence<byte> payload) => TSelf.CreateFrame(ResilientFrameKind.Message, payload);

   /// <summary>
   /// Optionally creates a message protocol frame with a payload memory block.
   /// </summary>
   public static virtual TSelf CreateMessage(ReadOnlyMemory<byte> payload) => TSelf.CreateFrame(ResilientFrameKind.Message, payload);

   /// <summary>
   /// Optionally creates a message protocol frame with a byte array payload.
   /// </summary>
   public static virtual TSelf CreateMessage(byte[] payload) => TSelf.CreateFrame(ResilientFrameKind.Message, payload);

   /// <summary>
   /// Gets the payload byte sequence of this frame if available.
   /// Defaults to empty sequence.
   /// </summary>
   public ReadOnlySequence<byte> GetPayloadSequence() => ReadOnlySequence<byte>.Empty;
