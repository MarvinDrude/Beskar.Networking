...
public static class Program
{
    ...
    {
        ConsoleRender.DrawHeader("Beskar Performance Shootout", "High-Performance Network Transport Shootout Benchmark", BoxStyle.Double, ConsoleColor.Cyan);
        ConsoleRender.WriteMarkupLine("Welcome to the high-performance network transport shootout benchmark!");
        ConsoleRender.WriteMarkupLine("This utility measures the maximum raw throughput over local loopback ([green]127.0.0.1[/]).\n");
        ...
        var selectedOption = ConsoleRender.AskChoice("Select action", choices, "a", vertical: true);
        if (selectedOption == "x")
        {
        ...
        if (!int.TryParse(durationStr, out var durationSec) || durationSec <= 0)
        {
            durationSec = 3;
        }

        var payloadKbStr = ConsoleRender.AskString("Enter payload block size (KB)", "64");
        if (!int.TryParse(payloadKbStr, out var payloadKb) || payloadKb <= 0)
        {
            payloadKb = 64;
        }
        ...
        if (runTcp)
        {
            tcpResult = await RunTcpBenchmarkAsync(durationSec, payloadSize);
        }

        if (runWs)
        {
            wsResult = await RunWsBenchmarkAsync(durationSec, payloadSize);
        }
    ...
    private static async Task<BenchmarkResult> RunTcpBenchmarkAsync(int durationSec, int payloadSize)
    {
        ...
        ConsoleRender.WriteMarkupLine($"[green]TCP Completed:[/] Transferred [white]{totalBytesRead / (1024.0 * 1024.0):N2} MB[/] in [white]{stopwatch.Elapsed.TotalSeconds:N2}s[/]\n");
    ...
    private static async Task<BenchmarkResult> RunWsBenchmarkAsync(int durationSec, int payloadSize)
    {
        ...
        ConsoleRender.WriteMarkupLine($"[green]WebSocket Completed:[/] Transferred [white]{totalBytesRead / (1024.0 * 1024.0):N2} MB[/] in [white]{stopwatch.Elapsed.TotalSeconds:N2}s[/]\n");
        return new BenchmarkResult(totalBytesRead, stopwatch.Elapsed.TotalSeconds);
    }
