...
public class MqttTrieSubscriptionRouterTests
{
   ...
   private static async Task AssertEnumerator(string topic, string[] expectedLevels)
   {
      ...
      {
         list.Add(Encoding.UTF8.GetString(enumerator.Current));
      }

      await Assert.That(list).Count().IsEqualTo(expectedLevels.Length);
      for (var i = 0; i < expectedLevels.Length; i++)
      {
         await Assert.That(list[i]).IsEqualTo(expectedLevels[i]);
      }
   }
   ...
   public async Task Route_ExactMatch_ShouldMatchCorrectly()
   {
      ...
      router.Subscribe(session1, "a/b/c"u8.ToArray(), QualityOfServiceType.AtLeastOnce, false, false, RetainHandlingType.SendAtSubscription, 0);
   ...
   public async Task Route_SingleLevelWildcard_ShouldMatchCorrectly()
   {
      ...
      router.Subscribe(session1, "a/+/c"u8.ToArray(), QualityOfServiceType.AtLeastOnce, false, false, RetainHandlingType.SendAtSubscription, 0);
   ...
   public async Task Route_MultiLevelWildcard_ShouldMatchCorrectly()
   {
      ...
      router.Subscribe(session1, "a/#"u8.ToArray(), QualityOfServiceType.AtLeastOnce, false, false, RetainHandlingType.SendAtSubscription, 0);
   ...
   public async Task Route_Unsubscribe_ShouldRemoveMatchingSubscription()
   {
      ...
      router.Subscribe(session1, filter, QualityOfServiceType.AtLeastOnce, false, false, RetainHandlingType.SendAtSubscription, 0);
   ...
   public async Task Route_UnsubscribeAll_ShouldRemoveAllSubscriptionsForSession()
   {
      ...
      router.Subscribe(session1, [.. "a/b/c"u8], QualityOfServiceType.AtLeastOnce, false, false, RetainHandlingType.SendAtSubscription, 0);
      router.Subscribe(session1, [.. "x/+/y"u8], QualityOfServiceType.ExactlyOnce, false, false, RetainHandlingType.SendAtSubscription, 0);
